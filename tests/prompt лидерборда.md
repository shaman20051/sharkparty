...
- список турниров - index_tournament.json
    (привести текст)
- список лидербордов - index_leaderboards.json
    (привести текст)
...
- файлы лидерборда со ссылкой на список турниров, участвующих в лидерборде - leaderboard_weekly.json, leaderboard_daily.json

- данные для лидерборда на вывод:
    (вставить из data_leaderboard_weekly.json)


- компонент Leaderboard.vue

*********

    1. место
    2. никнейм игрока, страна
    3. очки
    4. подробнее:
        1. № п/п
        2. название Турнира
        ...
        4. призовые
        5. очки ЛБ
- получаем данные навход (из перечня турниров):
Алгоритм:
- берем первый турнир из списка
- получаем данные по игроку из players.player
- добавляем в массив объектов leaderboard.players
    {
        [players: {
            "nickname": "Vasia",
            "country": "UA",
            },
        "idTurnament": " что является ИД?",
        "rank": "место".
        "points": 109
        ]
...
<a href="#" class="tournaments-grid__details-link" @click="$emit('select', t)">Подробнее</a>
```
в компоненте TournamentList.vue должен вызываться компонент Tournament.vue с сответствующим параметром, в котором передаются данные конкретного турнира из списка.
Но почему-то при нажатии любой из кнопок в списке турниров выводится всегда последний турнир из списка, а не тот на которм было нажатие.
В чем ошибка?